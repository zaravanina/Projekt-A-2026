...
<p><a href="/login">Gå til login</a></p>`,
  );
}
...
app.post("/api/login", async (req, res) => {
  ...
  db.prepare(
    "INSERT INTO login_links (user_id, token_hash, session_id, expires_at, created_at) VALUES (?, ?, ?, ?, ?)",
...
app.post("/register", async (req, res) => {
  ...
  if (existing) {
    return res
      .status(409)
      .send(`<p>Email already exists.</p><p><a href="/register">Try again</a></p>`);
  }
...
app.get("/magic/verify", (req, res) => {
  ...
  const row = db
    .prepare(
      "SELECT id, user_id, session_id, expires_at, used_at FROM login_links WHERE token_hash = ?",
    )
    .get(tokenHash) as
    | {
        id: number;
